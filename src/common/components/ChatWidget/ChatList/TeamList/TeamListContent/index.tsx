 ...
 */
const TeamListContent: React.FC<TeamListProps> = ({
  companyId = -1,
  modalItem = false,
  selectedMembers = [],
  ...
  const { companyTeamLists, loadNextPage, loadingList, loadingNextPage } =
    useContext(TeamListsContext);
  const teamList =
    companyTeamLists &&
    companyTeamLists.find((list) => list.companyId === companyId);
  ...
  const lastTeamElementRef = useCallback(
    (node) => {
      ...
      observer.current = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting && teamList?.hasMore) {
          loadNextPage();
    ...
    [loadingNextPage, teamList?.hasMore, loadNextPage]
  );
        ...
        <span>
          {teamList?.teams.map((team, index) => (
            <div
              key={index}
              ref={
                teamList?.teams.length === index + 1 ? lastTeamElementRef : null
              }
            >
              <TeamListItem
